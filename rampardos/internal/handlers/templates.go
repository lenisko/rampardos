...
// GetVersions handles GET /admin/api/templates/versions/:name
func (h *TemplatesHandler) GetVersions(w http.ResponseWriter, r *http.Request) {
	...
	versions, err := h.templatesController.GetTemplateVersions(name)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// GetVersionContent handles GET /admin/api/templates/version/:name/:timestamp
func (h *TemplatesHandler) GetVersionContent(w http.ResponseWriter, r *http.Request) {
	name := chi.URLParam(r, "name")
	if name == "" {
		http.Error(w, "Name parameter is required", http.StatusBadRequest)
		return
	...
	content, err := h.templatesController.GetTemplateVersionContent(name, timestamp)
	if err != nil {
		if os.IsNotExist(err) {
